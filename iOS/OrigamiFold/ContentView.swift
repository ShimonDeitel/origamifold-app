...
struct ContentView: View {
    ...
    @State private var editingItem: Fold?
    ...
    @ViewBuilder
    func row(for item: Fold) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(item.title)
                .font(Theme.headlineFont)
                .foregroundStyle(Theme.ink)
            Text("\(String(describing: item.paperType))  \u{00B7}  \(String(describing: item.difficulty))")
                .font(.caption)
...
struct ItemEditorView: View {
    ...
    let mode: EditorMode
    let onSave: (Fold) -> Void

    @State private var draft: Fold

    init(mode: EditorMode, onSave: @escaping (Fold) -> Void) {
        self.mode = mode
        self.onSave = onSave
        switch mode {
        case .add:
            _draft = State(initialValue: Fold(title: ""))
        case .edit(let item):
        ...
        NavigationStack {
            Form {
                Section("Fold") {
                    ...
                    TextField("Paper Type", text: $draft.paperType)
                        .accessibilityIdentifier("field.paperType")
            ...
            .navigationTitle(mode == .add ? "Add Fold" : "Edit Fold")
            .toolbar {
